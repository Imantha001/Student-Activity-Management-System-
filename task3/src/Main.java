...
public class Main {
        ...
        while (!choice.equals("9")) {
            ...
            switch (choice) {
                ...
                case "C":
                    ...
                    break;
                case "D":
                    generateCompleteReport(); // Generate complete report
                    break;
                case "9":
    ...
    private static void registerStudent() {
        // Check if the current student count is less than the 100
        if (studentCount < Max_Student) {
            ...
            // Create a new Student object with the entered ID, name and default marks
    ...
    private static void deleteStudent() {
        ...
        int index = -1;
        // Loop through the students array to find the student with the given ID
        ...
        // If the student is found (index is not -1)
        if (index != -1) {
    ...
    private static void findStudentByID() {
        ...
        for (int i = 0; i < studentCount; i++) {
            if (students[i].getStudentID().equals(idToFind)) {
                // If the student is found, display the student's name
    ...
    private static void storeStudentDataToFile() {
        try (PrintWriter writer = new PrintWriter(new FileWriter(Data_File))) {
            ...
            for (int i = 0; i < studentCount; i++) {
                // Write the student ID and name, separated by a comma
    ...
    private static void sortStudentsByName() {
        // Sort the array of students based on their names
    ...
    private static void addStudentResults() {
        ...
        // If student ID is not found display this message
        System.out.println("Student not found.");
    }
    ...
    private static void displayStudents() {
        ...
        for (int i = 0; i < studentCount; i++) {
            ...
            // Print average marks and overall grade
    ...
    // Generate a summary ot the system
    private static void generateSummary() {
        // Store total number of registered students
        int totalRegistrations = studentCount;
        // Students with more than 40 marks in all modules
        int studentsWithMoreThan40InAllModules = 0;

        // Loop through each student
        for (int i = 0; i < studentCount; i++) {
            // Get student modues
            Module[] modules = students[i].getModules();
            // Check if the student has more than 40 marks in all modules
            if (modules[0].getMarks() > 40 && modules[1].getMarks() > 40 && modules[2].getMarks() > 40) {
                studentsWithMoreThan40InAllModules++; // Increment the count
            }
        }

        System.out.println("\n----- System Summary -----");
        System.out.println("Total student registrations: " + totalRegistrations);
        System.out.println("Total students with more than 40 marks in all modules: " + studentsWithMoreThan40InAllModules);
    }
    ...
    private static void generateCompleteReport() {
        ...
        for (int i = 0; i < studentCount; i++) {
            ...
            int totalMarks = students[i].getModules()[0].getMarks() + students[i].getModules()[1].getMarks() + students[i].getModules()[2].getMarks();
            System.out.println(" Total Marks: " + totalMarks);
            // Average marks
            System.out.println(" Average Marks: " + students[i].getAverageMarks());
            // Overall grade
            System.out.println(" Overall Grade: " + students[i].getOverallGrade());
        }
    }
    ...
    // Bubble sort students by average marks in descending order
    private static void bubbleSortStudentsByAverageMarks() {
        boolean swapped;
        // Iterate through the student list multiple times
        for (int i = 0; i < studentCount - 1; i++) {
            swapped = false;
            // Compare  students
            for (int j = 0; j < studentCount - 1 - i; j++) {
                // If the current student has a lower average than the next student, swap
                if (students[j].getAverageMarks() < students[j + 1].getAverageMarks()) {
                    Student temp = students[j]; // Temporary variable
                    students[j] = students[j + 1]; // Move the next student to the current position
                    students[j + 1] = temp; // Move the current student to the next position
                    swapped = true;
                }
            }
